...
int main(int argc, char *argv[]) {
  ...
  if ((servSock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0)
    ...
    for (;;) { //Run forever
      ...
      // Receive echoString to recvbuffer
      while ((numBytes = recv(clntSock, recvbuffer, BUFSIZE - 1, 0)) > 0) {
        recvbuffer[numBytes] = '\0';
       // fputs(recvbuffer, stdout); // Prints recvbuffer 
      ...
      snprintf(sendbuffer, sizeof(sendbuffer), "%s\r\n", recvbuffer); // Store what's in recvbuffer to sendbuffer
      ssize_t numBytesSent = send(clntSock, sendbuffer, strlen(sendbuffer), 0); // Return echoString
      if (numBytesSent < 0)
