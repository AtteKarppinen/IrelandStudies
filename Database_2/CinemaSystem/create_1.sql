...
(
    ...
    ON UPDATE CASCADE   -- Update reference when movie gets updated
    ON DELETE CASCADE   -- No movie so no show. Cascade on deletion
);
...
(
    ...
    ON UPDATE CASCADE   -- If show is relocated, update sale infromation too
    ON DELETE SET NULL  -- Save sales, viewing won't matter so set null
);
...
ON UPDATE CASCADE       -- If show is relocated to new screen, update viewing
ON DELETE CASCADE;      -- If screen is out of comission, cancel (delete) viewing

ALTER TABLE sales
ADD FOREIGN KEY(customer_id) REFERENCES customers
ON UPDATE CASCADE       -- Update if customer gets updated
ON DELETE SET NULL;     -- Save sales, set customer to null
