...
public class DetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(com.example.attek.smurfs.R.layout.activity_details);

        Intent intent = getIntent();
        int id = intent.getIntExtra("smurf_id", com.example.attek.smurfs.R.id.jokey);
        String text = "";
        if (id == com.example.attek.smurfs.R.id.jokey) {
            text = SMURF_DETAILS[0];
        } else if (id == com.example.attek.smurfs.R.id.hefty) {
            text = SMURF_DETAILS[1];
        } else if (id == com.example.attek.smurfs.R.id.nerdy) {
            text = SMURF_DETAILS[2];
        } else { // if (id == R.id.raph)
            text = SMURF_DETAILS[3];
        }
        TextView tv = (TextView) findViewById(com.example.attek.smurfs.R.id.smurf_info);
        tv.setText(text);
    }
